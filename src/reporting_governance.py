...
def _build_readme(metrics: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- CBM vs reactiva: **+{_format_float(float(metrics['cbm_vs_reactiva_availability_pp']), 2)} p.p.** en disponibilidad proxy",
            f"- ahorro operativo proxy CBM vs reactiva: **~{_format_million_eur(float(metrics['cbm_operational_savings_eur']), 2)}M EUR**",
            (
                "- rango plausible de ahorro CBM vs reactiva (P10-P90): "
                f"**~{_format_million_eur(float(metrics['cbm_value_range_min_eur']), 2)}M a "
                f"{_format_million_eur(float(metrics['cbm_value_range_max_eur']), 2)}M EUR**"
            ),
            (
                "- robustez del caso CBM (simulaciones con ahorro positivo): "
                f"**{_format_float(float(metrics['cbm_prob_positive_savings']) * 100, 1)}%**"
            ),
            f"- horas potencialmente evitables por inspección automática: **~{_format_float(float(metrics['avoidable_downtime_hours_inspection']), 0)} h**",
            "",
            "## Recommendations",
            ...
            f"- **Componente que debe sustituirse primero:** `{metrics['top_component_by_priority']}` (familia `{metrics['top_component_family_by_priority']}`)",
            (
                "- **Impacto operativo de intervenir ahora:** "
                f"reducción esperada de downtime frente a diferir 14 días (~{_format_float(float(metrics['deferral_downtime_delta_14d_h']), 1)} h)."
            ),
            (
            "- **Riesgo de retrasar la intervención:** "
                f"incremento de coste proxy de ~{_format_million_eur(float(metrics['deferral_cost_delta_14d_eur']), 2)}M EUR a 14 días."
            ),
            (
                "- **Cuándo CBM genera más valor:** "
                f"cuando hay saturación de depósito alta (actualmente {metrics['top_depot_by_saturation']} "
                f"con {_format_float(float(metrics['top_depot_saturation_pct']), 1)}%) y señales tempranas accionables."
            ),
            (
                "- **Nota de gobierno de KPI:** backlog crítico del README se refiere a backlog físico "
                "(edad/severidad), no a riesgo de diferimiento."
            ),
        ]
    )
