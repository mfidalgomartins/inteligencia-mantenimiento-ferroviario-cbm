...
def _metric_contracts() -> list[MetricContract]:
    return [
        ...
        MetricContract(
            ...
            definition="Probabilidad aproximada de fallo a corto plazo por componente.",
            formula="Percentil calibrado de signal_risk con ajustes por familia, estrés y restauración.",
            valid_grain="componente_corte",
            unit="ratio_0_1",
            owner="Analítica de fiabilidad",
            ...
            anti_interpretation="No interpretar causalidad directa; es puntuación de propensión, no modelo físico causal.",
            source_of_truth="data/processed/scoring_componentes.csv",
            maturity="alta",
        ),
        MetricContract(
...
def _assert_governance_compliance(checks_df: pd.DataFrame) -> None:
    ...
    raise RuntimeError(f"Bloqueos de contrato de gobernanza detectados ({len(blockers)}). Principales incidencias: {short}")
