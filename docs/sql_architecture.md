...
## Dialecto elegido
Se utiliza **DuckDB SQL** por su capacidad analítica local, soporte de funciones de ventana, CTEs avanzados y ejecución reproducible sobre CSV/Parquet sin infraestructura adicional.
...
4. consultas de indicadores: vistas listas para explotación ejecutiva.
5. consultas de validación: controles de calidad y coherencia analítica.
...
- `03_staging_maintenance.sql`: fallas, mantenimiento, alertas, pendientes, operación.
- `04_integrated_component_health.sql`: entradas de salud por componente-día.
...
- `07_analytical_mart_component_day.sql`: tabla analítica componente-día + `vw_component_daily_health`.
- `08_analytical_mart_unit_day.sql`: tabla analítica unidad-día + `vw_unit_operational_risk` + `vw_depot_maintenance_pressure`.
- `09_analytical_mart_fleet_week.sql`: tabla analítica flota-semana.
- `10_kpi_queries.sql`: indicadores de riesgo, saturación, indisponibilidad, inspección automática y valor CBM.
- `11_validation_queries.sql`: validaciones de nulos, rangos y coherencia.

## Orden de ejecución
1. staging (`01` a `03`)
2. integration (`04` a `06`)
3. tablas analíticas (`07` a `09`)
4. indicadores (`10`)
5. validaciones (`11`)

## Contratos de grano y uniones
...
## Validaciones SQL
- `val_row_counts`: confirma que las tablas analíticas principales no quedan vacías.
- `val_null_rates_critical`: nulos en claves y métricas críticas.
...
- `val_backlog_semantic_consistency`: coherencia de pendientes físicos, ratios y exposición.
- `val_semantic_health_deterioration`: balance obligatorio salud + deterioro = 100.
- `val_primary_key_uniqueness`: duplicados en claves naturales y granos de tablas analíticas.
- `val_referential_integrity`: órfãos y inconsistencias unidad/componente/flota.
- `val_join_cardinality`: claves de unión que pueden duplicar filas aguas abajo.
- `val_metric_ranges`: límites de métricas normalizadas `[0,1]` y `[0,100]`.
- `val_business_metric_coherence`: reconciliación de horas de servicio y aproximaciones de fiabilidad/valor.

## Desempeño e índices sugeridos
DuckDB ejecuta esta capa en modo analítico local y no necesita índices persistentes para el volumen sintético actual. Si se porta a PostgreSQL, SQL Server o un almacén analítico con tablas físicas, priorizar índices/claves de agrupación en:
